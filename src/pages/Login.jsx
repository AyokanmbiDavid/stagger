...
const Login = () => {
  ...
  const showToast = (message, type = 'error', ms = 2800) => {
    ...
    <>
      <Suspense fallback={<LoadingBig/>}>
        <div className="py-2 h-screen w-full relative">
          {toastNotice && (
            <div className={`fixed top-6 right-6 z-50 transform transition-transform duration-300 ease-out ${toastVisible ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0'} ${toastNotice.type === 'success' ? 'bg-green-500 border-2 border-green-600 border-b-3 boder-b-green-700' : 'bg-red-500 border-2 border-b-3 border-red-600 border-b-red-700'} text-white px-4 py-2 rounded-xl shadow-lg`}> 
              {toastNotice.message}
            </div>
          )}
          {/* title */}
                    ...
                    transition={{duration: 0.6}}
                     className="md:hidden p-2 absolute w-full bottom-5 border-5 border-slate-300 rounded-4xl min-h-[420px] shadow-lg duration-200">
                      ...
                      onSubmit={submitForm}
                       ...
                       {/* login button */}
                       <Link to={'/'}>
                           <button className='w-full border-2 border-b-4 rounded-2xl p-3 mt-3 border-blue-600'> Register
                           </button>
                       </Link>
                    </motion.div>
        </div>
